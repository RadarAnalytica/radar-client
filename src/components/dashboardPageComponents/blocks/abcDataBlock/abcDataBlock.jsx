                ...
                <div className={styles.block__table}>
                    ...
                    <div className={styles.block__tableRow}>
                        ...
                        <div className={styles.block__tableCell}>
                            ...
                            <span style={{'color': 'rgba(0, 182, 155, 1)', 'backgroundColor': 'rgba(0, 182, 155, 0.1)'}} className={styles.block__tableCellPercentData}>{formatPrice(data.amountPercentA, '%', true)}</span>
                        </div>
                        <div className={styles.block__tableCell}>
                            <span className={styles.block__tableCellMainData}>{formatPrice(data.amountB, '₽')}</span>
                            <span style={{'color': '#F0AD00', 'backgroundColor': '#F0AD000D'}} className={styles.block__tableCellPercentData}>{formatPrice(data.amountPercentB, '%', true)}</span>
                        </div>
                        <div className={styles.block__tableCell}>
                            <span className={styles.block__tableCellMainData}>{formatPrice(data.amountC, '₽')}</span>
                            <span style={{'color': 'rgba(249, 60, 101, 1)', 'backgroundColor': 'rgba(249, 60, 101, 0.1)'}} className={styles.block__tableCellPercentData}>{formatPrice(data.amountPercentC, '%', true)}</span>
                        </div>
                    </div>
                    <div className={styles.block__tableRow}>
                        ...
                        <div className={styles.block__tableCell}>
                            ...
                            <span style={{'color': 'rgba(0, 182, 155, 1)', 'backgroundColor': 'rgba(0, 182, 155, 0.1)'}} className={styles.block__tableCellPercentData}>{formatPrice(data.countPercentA, '%', true)}</span>
                        </div>
                        <div className={styles.block__tableCell}>
                            <span className={styles.block__tableCellMainData}>{formatPrice(data.countB, 'шт')}</span>
                            <span style={{'color': '#F0AD00', 'backgroundColor': '#F0AD000D'}} className={styles.block__tableCellPercentData}>{formatPrice(data.countPercentB, '%', true)}</span>
                        </div>
                        <div className={styles.block__tableCell}>
                            <span className={styles.block__tableCellMainData}>{formatPrice(data.countC, 'шт')}</span>
                            <span style={{'color': 'rgba(249, 60, 101, 1)', 'backgroundColor': 'rgba(249, 60, 101, 0.1)'}} className={styles.block__tableCellPercentData}>{formatPrice(data.countPercentC, '%', true)}</span>
                        </div>
                    </div>
                </div>
            }
