    ...
    afterEvent: (chart: Chart, args: any) => {
      ...
      if (activeElements.length > 0) {
        ...
        if (item) {
          ...
          // В Chart.js event.x и event.y - это координаты относительно canvas
          // getBoundingClientRect() возвращает координаты canvas относительно viewport
          // Поэтому canvasPosition.top + event.y дает правильную координату Y относительно viewport
          const mouseX = canvasPosition.left + event.x;
          const mouseY = canvasPosition.top + event.y;
