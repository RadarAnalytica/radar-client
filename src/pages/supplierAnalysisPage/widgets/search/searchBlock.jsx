...
const SearchBlock = () => {
    ...
    const historyButtonClickHandler = (e) => {
        const { id } = e.target;
        setInputValue(id)
        navigate(`/supplier-analysis/${id}`)
    }

    const searchSubmitHandler = (e) => {
        if (e && e.key && e.key !== 'Enter') return
        let normilizedId = inputValue
        if ((normilizedId.includes('http://') || normilizedId.includes('https://')) && normilizedId.includes('wildberries')) {
            const startId = inputValue.indexOf('seller') + 'seller'.length + 1;
            if (inputValue.indexOf('seller') === -1) {
                setRequestStatus({ ...requestInitState, isError: true, message: 'Не верный формат ссылки. Вставьте ссылку вида: https://www.wildberries.ru/seller/ID' })
                setInputValue('')
                return
            }
            normilizedId = normilizedId.substring(startId, normilizedId.length)
        }
        if ((normilizedId.includes('http://') || normilizedId.includes('https://')) && !normilizedId.includes('wildberries')) {
            const startId = inputValue.indexOf('seller') + 'seller'.length + 1;
            if (inputValue.indexOf('seller') === -1) {
        ...
        // } else {
        //     const startId = inputValue.indexOf('wildberries.ru/seller/') + 'wildberries.ru/seller/'.length
        //     if (startId === -1) {
        //         setRequestStatus({ ...requestInitState, isError: true, message: 'Не верный формат артикула. Вставьте только числа или ссылку вида: https://www.wildberries.ru/seller/ID' })
        //         setInputValue('')
        //         return
        //     }
        //     normilizedId = inputValue.substring(startId, -1)
        // }

        navigate(`/supplier-analysis/${normilizedId}`)
    }
