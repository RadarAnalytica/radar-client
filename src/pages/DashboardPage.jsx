...
const DashboardPage = () => {
  ...
  const storeData = [
    ...
    {
      name: 'Едет к клиенту',
      initialPrice: content?.toClient?.reduce(
        (acc, el) =>
          acc +
          el.inWayToClient *
      ...
      salesPrice:
        content?.toClient?.reduce(
          (acc, el) => acc + el.inWayToClient * el.Price,
          0
        ) || 0,
      quantity:
        content?.toClient?.reduce((acc, el) => acc + el.inWayToClient, 0) || 0,
    },
    {
      name: 'Едет от клиента',
      initialPrice: content?.fromClient?.reduce(
        (acc, el) =>
          acc +
          el.inWayToClient *
            stockAndCostsMatch?.find((item) => item.nmID === el.nmId)
      ...
      salesPrice:
        content?.fromClient?.reduce(
          (acc, el) => acc + el.inWayFromClient * el.Price,
          0
        ) || 0,
      quantity:
        content?.fromClient?.reduce((acc, el) => acc + el.inWayFromClient, 0) ||
        0,
    },
    {
  ...
  const costsData = [
    {
      ...
      percentRate2:
        content?.advertisment?.growthPercentageExpensesPercentage || '0',
    },
    {
      ...
      percentRate2:
        content?.commissionFromProfit?.commissionPercentGrowth || '0',
    },
    {
  ...
  const financeData = [
    ...
    },
    {
      name: 'Налог',
      amount: tax
        ? (curOrders?.selectedPeriod?.buyoutsSumRub / 100) * tax.value
        : '0',
      rate: content?.tax?.percent || '0',
    },
    {
      name: 'Чистая прибыль',
      amount: content?.netProfit?.sum
        ? content?.netProfit?.sum
        : tax
  ...
  const profitabilityData = [
    ...
    },
    {
      name: 'ROI',
      value:
        ((curOrders?.selectedPeriod?.buyoutsSumRub - salesSelfCost) /
          salesSelfCost) *
          100 || '0',
    },
    {
            ...
            <div>
              <div className='container dash-container p-3 pt-0 d-flex gap-3'>
                <MediumPlate
                  ...
                  percent2={
                    curOrders?.periodComparison?.avgOrdersCountPerDayDynamics
                  }
                  text={
                    curOrders?.selectedPeriod?.avgPriceRub *
                    curOrders?.selectedPeriod?.avgOrdersCountPerDay
                  }
                ...
                <div className='col d-flex flex-column' style={{ gap: '2vh' }}>
                  <div className='' style={{ height: '11vh' }}>
                    <SmallPlate
                      name={'Процент выкупа'}
                      value={
                        curOrders?.selectedPeriod?.conversions?.buyoutsPercent
                      }
                      type={'percent'}
                      percent={
                        curOrders?.periodComparison?.conversions
                          ?.buyoutsPercent || '0'
                      }
                    />
                  </div>
              ...
              <div className='container dash-container p-4 pt-0 d-flex gap-3'>
                <div className='col' style={{ height: '14vh' }}>
                  <SmallPlate
                    ...
                    name={'Комиссия WB'}
                    value={
                      content?.wbComission?.currentPeriodCommission * -1 || '0'
                    }
                    type={'price'}
