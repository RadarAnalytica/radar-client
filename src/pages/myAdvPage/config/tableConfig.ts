  ...
  const columns: ColumnConfig[] = [
    {
      ...
      groupColor: 'white',
      children: [
        {title: 'Название кампании', dataIndex: 'company_name', undraggable: true, canToggle: false, sortable: false, fixed: true, width: 240},
    ...
    },
    {
      ...
      groupColor: 'white',
      children: [
        {title: 'Корзина', dataIndex: 'cart', sortable: true, fixed: false, width: 50, tooltipText: 'Сколько раз товар был добавлен в корзину после клика по рекламе. Показывает рост интереса и намерение купить'},
        {title: 'Заказы', dataIndex: 'orders', sortable: true, fixed: false, width: 50, tooltipText: 'Общее число оформленных заказов, атрибутированных к данной рекламной кампании'},
        {title: 'Заказано, шт', dataIndex: 'order_item_count', sortable: true, fixed: false, width: 50, tooltipText: 'Суммарное количество единиц товара во всех заказах, привлеченных этой кампанией'},
        {title: 'Прогноз выкуп, шт', dataIndex: 'expected_purchase', sortable: true, fixed: false, width: 50, tooltipText: 'Ожидаемое количество единиц товара, которые будут оплачены. Для точного расчёта используется финальный процент выкупа на основе данных 14-дневной давности, когда все заказы уже перешли в статус «Выкуплен» или «Возврат/Невыкуп». Это исключает влияние незавершённых заказов на статистику'},
        {title: 'Просмотры → Клик (CTR)', dataIndex: 'view_click', sortable: true, fixed: false, width: 50, tooltipText: 'Коэффициент кликабельности объявления. Показывает, какая доля показов привлекла внимание пользователя и вызвала переход. Ключевой показатель релевантности рекламного объявления'},
        {title: 'Клик → Корзина', dataIndex: 'click_cart', sortable: true, fixed: false, width: 50, tooltipText: 'Доля пользователей, которые после перехода на карточку проявили серьезное намерение к покупке, добавив товар в корзину. Говорит о качестве карточки и оффера'},
    ...
    },
    {
      ...
      groupColor: 'white',
      children: [
        {title: 'Просмотры', dataIndex: 'views', sortable: true, fixed: false, width: 50, tooltipText: 'Сколько раз ваше рекламное объявление было показано пользователям площадки в выбранном периоде. Базовый показатель охвата аудитории'},
        {title: 'Клики', dataIndex: 'clicks', sortable: true, fixed: false, width: 50, tooltipText: 'Число переходов пользователей на вашу карточку товара после показа рекламного объявления. Отражает уровень интереса'},
        {title: 'CPC', dataIndex: 'cpc', sortable: true, fixed: false, width: 50, tooltipText: 'Рассчитывается как Расходы на рекламу / Клики. Показатель того, сколько в среднем вы платите за один переход на карточку'}, 
        {title: 'Средний CPM за период', dataIndex: 'avg_cpm', sortable: true, fixed: false, width: 50, tooltipText: 'Средняя стоимость 1000 показов рекламного объявления в рамках данной рекламной кампании за выбранный период. Показатель помогает понять, сколько стоит охват аудитории на уровне показов, независимо от кликов или других действий. Формула: (Расходы на рекламу / Просмотры) * 1000'},
        ...
        {title: 'ДРР заказы', dataIndex: 'drr_orders', sortable: true, fixed: false, width: 50, tooltipText: 'Показывает, какой процент от суммы всех оформленных заказов составляют затраты на рекламу. Помогает оценить рентабельность привлечения заказов. Формула: (Расходы на рекламу / Сумма заказов) * 100%'},
        {title: 'Прогноз ДРР (по выкупу)', dataIndex: 'drr_purchase', sortable: true, fixed: false, width: 50, tooltipText: 'Ожидаемый процент рекламных затрат от суммы прогнозируемой выручки. Более точный показатель рентабельности инвестиций в рекламу (ROAS), так как основан на прогнозе реальных продаж. Формула: (Расходы на рекламу / Прогноз выкупа на сумму) * 100%'},
        {title: 'CPCart', dataIndex: 'cp_cart', sortable: true, fixed: false, width: 50, tooltipText: 'Средние затраты на то, чтобы пользователь добавил товар в корзину. Оценивает эффективность рекламы на этапе формирования спроса. Формула: Расходы на рекламу / Добавления в корзину'},
        {title: 'CPO', dataIndex: 'cpo', sortable: true, fixed: false, width: 50, tooltipText: 'Средние рекламные расходы, которые привели к оформлению одного заказа. Формула: Расходы на рекламу / Заказы'},
        {title: 'Прогноз CPS', dataIndex: 'expected_cps', sortable: true, fixed: false, width: 50, tooltipText: 'Оценка средних рекламных затрат на одну фактическую продажу (выкуп). Учитывает прогнозируемое количество выкупов. Формула: Расходы на рекламу / Прогноз выкупа (шт)'},
